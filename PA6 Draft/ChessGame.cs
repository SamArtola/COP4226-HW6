...
namespace PA6_Draft
{
    ...
    class Move
    {
        ...
        public override string ToString()
        {
            ...
            if (Castled == Castle.BLONG || Castled == Castle.WLONG)
                result += "O-O-O";
            else if (Castled == Castle.BSHORT || Castled == Castle.WSHORT)
                result += "O-O";
            else
            {
                switch (MovedPiece)
    ...
    class ChessGame: INotifyPropertyChanged
    {
        internal event ChessEvent Promote;
        internal event ChessEvent Moved;
        internal event ChessEvent Castled;
        ...
        internal bool AutoPromote = false;
        internal long WLimit;
        internal long BLimit;
        internal string Player1Name;
        internal string Player2Name;
        private string whitetimeLimit;
        public string WhiteTimeLimit
        {
            get { return whitetimeLimit; }
            set
            {
                whitetimeLimit = value;
                InvokePropertyChanged(new PropertyChangedEventArgs("WhiteTimeLimit"));
            }
        }
        private string blacktimeLimit;
        public string BlackTimeLimit
        {
            get { return blacktimeLimit; }
            set
            {
                blacktimeLimit = value;
                InvokePropertyChanged(new PropertyChangedEventArgs("BlackTimeLimit"));
            }
        }
        ...
        internal bool Move(Move move)//If the move is not legal, returns false, otherwise, it makes the move and returns true...
        {
            ...
            switch (move.MovedPiece)
            {
                case Piece.BPAWN:
                    ...
                    if (y1 == 6)//promotion
                    {
                        Board[x1][y1].Occupant = Piece.BQUEEN;
                        move.Promoted = Promotion.BQUEEN;
                        if (!AutoPromote)
                            Promote(move);
                    }
                    break;
                case Piece.WPAWN:
                    ...
                    if (y1 == 1)//promotion
                    {
                        Board[x1][y1].Occupant = Piece.WQUEEN;
                        move.Promoted = Promotion.WQUEEN;
                        if (!AutoPromote)
                            Promote(move);
                    }
                    break;
            ...
            if (!AutoPromote && move.Promoted != Promotion.NONE)//move is not completed yet. waiting for user's choice of promoted piece...
            {
                //Board[x1][y1].Occupant = ((int)move.Promoted % 2 == 0) ? Piece.BPAWN : Piece.WPAWN;
                Board[x2][y2].Occupant = move.CapturedPiece;
                return true;
            }
        ...
        private void SetMovesToString(Move move)
        {
            if (move.MovedPiece == Piece.BKING || move.MovedPiece == Piece.WKING ||
                move.MovedPiece == Piece.BPAWN || move.MovedPiece == Piece.WPAWN)
                return;
            Board[move.X2][move.Y2].Occupant = move.CapturedPiece;
            Board[move.X1][move.Y1].Occupant = move.MovedPiece;
            bool sameFile = false, sameRank = false,duplicate = false;
            for(int i = 0; i < 8; i++)
                for(int j =0;j < 8; j++)
                    if((i!= move.X1 || j != move.Y1) && Board[i][j].Occupant == move.MovedPiece)
                        if(LegalMove(new Move(i, j, move.X2, move.Y2), true))
                            if (TryLegalMove(new Move(i, j, move.X2, move.Y2), WhiteTurn))
                            {
                                duplicate = true;
                                sameFile = sameFile || i == move.X1;
                                sameRank = sameRank || j == move.Y1;
                            }
            Board[move.X2][move.Y2].Occupant = move.MovedPiece;
            Board[move.X1][move.Y1].Occupant = Piece.NONE;
            if (!duplicate)
                return;
            string temp = move.ToString();
            int sep = (int)move.MovedPiece % 2 == 0 ? 3 : 1;
            if (!sameFile)
                move.toString = temp.Substring(0, sep) + Board[move.X1][move.Y1].File + temp.Substring(sep, temp.Length - sep);
            else if(!sameRank)
                move.toString = temp.Substring(0, sep) + Board[move.X1][move.Y1].Rank + temp.Substring(sep, temp.Length - sep);
            else
                move.toString = temp.Substring(0, sep) + Board[move.X1][move.Y1].File+ Board[move.X1][move.Y1].Rank + temp.Substring(sep, temp.Length - sep);
        }

        internal void CompletePromotionMove(Move move)
        {
            Board[move.X2][move.Y2].Occupant = (Piece)move.Promoted;
            EnPassant = null;
            if (WhiteTurn)
                WhiteTimeLimit = TimeToString(WLimit += Increment);
            else
